...
int main(int argc, const char **argv) {
  ...
  std::map<int, uint64_t> callstack_depths;
  ...
  while ((dp = readdir(dirp)) != NULL) {
    ...
    while (LoadNextRecord(f, NULL, &ld)) {
      ...
      if (ld.stack_trace_size() > 0) {
        int module_idx = ld.stack_trace(0).module_idx();

        bool consistent = true;
        for (int i = 1; i < ld.stack_trace_size() - 1; i++) {
          if (ld.stack_trace(i).module_idx() != module_idx) {
            consistent = false;
            break;
          }
        }
        ...
        if (consistent) {
          ...
          if (globals::unique_traces.find(signature) == globals::unique_traces.end()) {
            // Save information about the callstack depth.
            callstack_depths[ld.stack_trace_size()]++;
  ...
  printf("--------------------------------------- Depths:\n");
  for (const auto& it : callstack_depths) {
    printf("%.2d: %10llu\n", it.first, it.second);
  }
